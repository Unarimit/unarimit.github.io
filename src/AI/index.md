...
- 行为树
    - 入门相对简单（教程和实现丰富，更符合人的直觉），容易可视化，应该是主流选择。
...
- HTN: Hierarchical Task Network, 分层任务网络
    - 为了解决行为树从设计层面上不直观的问题（对程序友好，但相对的看懂一颗复杂子树在干什么很费劲），对任务进行抽象
    - 分层体现在需要先通过策略选择一个计划再执行一系列任务，计划和任务是上下层的关系。
- GOAP：Goal Oriented Action Planner, 目标导向型行为规划
    - 在具体的任务执行上，和HTN的情况相似
    - 基于图的行为规划，把每次规划抽象为求最小路径。
    - 难点在于如何把世界状态抽象为边的权值，节点变多之后产生的决策很难在设计时预测。
- 蒙特卡洛（MCTS）：通过搜索树的形式，选取较优的节点
    - 适合：围棋、全面战争系列游戏等
- *Reinforcement Learning（alpha go、alpha star）-> LLM-based Agent (AI方法不断进化中...)

以上几种方法，HTN、GOAP是注重设计表达的方法，更注重规划，而状态机和行为树则更像是结构化、工程化的 “类 if else” 。HTN和GOAP也可以在具体任务执行时使用行为树。
...
## 参考
### 视频
- [GAMES104-现代游戏引擎：从入门到实践，第十六讲](https://www.bilibili.com/video/BV1r34y1J7Sg)
...
- [ARTIFICIAL INTELLIGENCE FOR GAMES - IAN MILLINGTON](https://theswissbay.ch/pdf/Gentoomen%20Library/Game%20Development/Programming/Artificial%20Intelligence%20for%20Games.pdf)，偏实践
