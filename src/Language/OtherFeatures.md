...
> 有栈协程和无栈协程的区别主要在于协程的执行栈是由谁分配和管理的。在有栈协程中，操作系统为每个协程分配一个固定大小的栈空间，由协程自己管理和使用。在无栈协程中，协程的执行栈是动态分配的（如一个可迭代对象），可以在运行时根据需要进行扩展或收缩，也可以与其他协程共享同一个栈空间。
...
> 异步方法无法声明 in、ref 或 out 参数，但可以调用包含此类参数的方法。 同样，异步方法无法通过引用返回值，但可以调用包含 ref 返回值的方法。--[参考](https://learn.microsoft.com/zh-cn/dotnet/csharp/asynchronous-programming/task-asynchronous-programming-model)
...
- [GAMES104现代游戏引擎课程的第十四讲-bilibili](https://www.bilibili.com/video/BV1QN4y1u78P)中，描述了自建引擎时，如何在c++中，通过在编译中AST（抽象语法树）生成阶段附加生成描述文件+mustache生成字段get、set代码的方式实现反射功能
...
通过定义程序集，可以组织代码以促进模块化和可重用性。为项目定义的程序集中的脚本不再添加到默认程序集中，并且只能访问指定的其他程序集中的脚本。[^程序集定义]
:::
...
[^程序集定义]: [程序集定义 - Unity Doc](https://docs.unity3d.com/cn/2022.3/Manual/ScriptCompilationAssemblyDefinitionFiles.html)