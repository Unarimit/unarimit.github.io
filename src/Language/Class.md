...
-  `T&&`
    - 作为确定类型的形参时（如 `int&&`），只接受右值实参（临时量或转化成的）。
    - 作为模板形参时，因为模板形参的**引用折叠**规则，表示“万能引用”，即可以根据实参自行推断出 `T&&`、`T&`、`const T&`（当存在重载版本，且该类型为`const T&`时，按照匹配优先级，`T&&`将只匹配右值(移动语义)，可以参考[`vector<T>` 的实现](./Template.md#试写-vector-t)）。
...
这个话题由于牵扯到工程问题，会比较复杂。先以关键字为出发点简单涉及一下：
