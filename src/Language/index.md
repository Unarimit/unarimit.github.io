...
本版块将重点描述C#（7.0）和C++（11）两种编程语言的学习路径，参考部分也给出了一些书目。

> 之所以C#和C++并列，是因为这两个语言在Unity中都会接触到，做对比学习用途。
...
- 从面试的角度上说：C++因为比较基础，有很多面经，在学习时可以根据面经检测是否学的牢固（指满足应聘方的要求）。而C#相比C++虽然可以问的角度很多，但其相比其他语言独特的那些东西（托管内存、JIT、泛型、委托、一系列语法糖等）应该总是会被问到的。
