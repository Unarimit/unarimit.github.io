...
> 补：作为解释型语言，不需要等编译和unity烦人的reload domain，小范围修改更快速和舒服
...
#### Q4：如何解耦逻辑，不产生超过400行的lua类

由于没有像C#的partial类了，通过拆文件的方式往上叠逻辑没那么方便了。
> 当然，partial类如果项目没有规范乱用的话也不太好

好在lua不原生支持私有，所以可以通过组合的方式拉屎，因为所有字段和方法都是public。进一步，这里可以借鉴ECS模式，将数据和逻辑拆分开，逻辑再进一步按类型拆分。

不过项目没有ECS规范时，还是用组合的方式，主类放数据定义和入口方法，组合类按逻辑类型（动画特效、模式2、和其他模块的耦合逻辑等）拆分后在主类调用就行。
