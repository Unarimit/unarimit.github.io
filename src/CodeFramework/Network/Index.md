...
内容包括从基础网络协议的`TCP`、`UDP`再到帧同步、状态同步这类网络同步常用方法。

## 协议的选择

根据游戏类型的不同选择合适的网络协议：
- 对于实时性要求比较高的游戏会优先选择`UDP`（如csgo、守望先锋）
- 策略类的游戏则会考虑使用`TCP`（如炉石传说）
...
`TCP`中，就通过滑动窗口和对应的ARQ机制保证（等待超时重传、回退窗口左标重传、指定重传三个方案可供选择）

如果使用`UDP`等不可靠的传输协议，就需要另外实现ARQ，`KCP`【2】是一个很好的例子
...
一个复杂的多人战斗场景（如moba、fps）可能会状态同步和帧同步一起使用
...
有内插值（Interpolation）和外插值（Extrapolation）两种插值方式，根据不同场景使用不同的方式。

![alt text](image-1.png)

一般情况下，fps、moba等角色控制的移动不符合物理的使用内插值，载具等移动符合物理的使用外插值。所以可能出现一个游戏中内插值外插值混用的情况（如PUBG）
- 因为不符合物理的运动无法根据速度/加速度的关系推断出合理的后插值
- 无论哪种插值都可能出现服务器和解问题，例如高延迟玩家后手打断先手的操作

## 命中判定——延迟时的交互结算的妥协

fps场景中的命中判定，是一个经典的因延迟影响，不得不将交互结算逻辑复杂化的例子。

> 在moba等游戏类型中，攻击前摇可以缓解类似的问题。


![alt text](image-4.png)

有两个流派
- 客户端结算：如果客户端判定命中，直接把本地结算，并上报给服务端命中信息（客户端tick，敌我位置等信息）。服务端在简单raycast校验无误之后接受结果。
    - 容易作弊
- 服务端结算：如果客户端判定命中，先上报服务端命中信息。服务端在通过延迟取快照计算后返回给客户端命中结果。
    - 结算存在时延，客户端需要先播放特效优先给玩家反馈。
