...
- 帧同步（lockstep synchronization）：将所有客户端操作汇总排序，下发给客户端模拟
    ...
    - 一致性保证很难：只对关键逻辑数据确保一致性（如浮点数->定点数）。特效和渲染之类的显示方面差一点影响不明显
    - 客户端拥有所有数据，作弊相对简单。
    - 断线重连：追帧 + 快照优化。这些技术可以拓展到回放和观战。
    - *需要模拟关键信息做校验，不必要不需发给客户端
- 状态同步（state synchronization）
    - 和帧同步最大的区别在于，所有世界模拟和逻辑都发生在中心服务器
    - 因为状态需要等服务器回传会比较慢，所以客户端一般会做帧预测，使玩家移动平滑（见经典的小球移动问题【1】、【4】）
        - 预测一定会产生很多问题，所以涉及服务器和解（Server Reconciliation）等机制
