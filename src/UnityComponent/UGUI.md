...
性能优化相关可以参考[性能优化-UI篇](../Profiler/ProfilerUI/index.md)

::: tip
UGUI的代码是公开的，想要了解组件是如何工作的可以看看。（例如`GridLayoutGroup`是如何计算每个ui的位置的）

但具体到渲染时的代码时不公开的，例如合批机制。
:::
...
`RenderMode`中，一般会把 永远显示在屏幕前（Screen Space - Overlay）和显示在相机前（Screen Space - Camera）区别讨论。他们的用途是相似的，Overlay也可以和 `renderTexture` 叠加实现相同的事。但 `renderTexture` 开销相对大，所以基本会用 显示在相机前（Screen Space - Camera） 作为一般UI画布的设置。

> 当然，为了实现一些功能， `renderTexture` 也经常配合各种画布使用。如3D场景的异形显示、UI 局部特效与遮罩、复杂的 3D 角色/模型交互展示等。
