...
- fbx导入unity
    - 会出现骨骼格式不对的问题（Transform、BindPose 矩阵，以及 Mesh（顶点数据）任意一项不匹配），无法匹配vrm导入产生的humanoid，这里写一个[脚本](./SkeletonBindingFixer.cs)矫正
...
::: details ai解释
为什么“前伸”会导致穿模？
在 T-Pose 下，手臂骨骼（UpperArm）的初始角度是 $0^\circ$。
- 前伸动作： 手臂向前方旋转约 $90^\circ$。
