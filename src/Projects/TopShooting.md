...
MyTDS作为校招项目已经完成了他的使命，由于对一开始几个点的不满意，2024年之后的开发基本没有进展。而且在我从事开发两年，再回顾自己写的代码，很多地方代码写的不太行，很多地方设计的也不太行。

总而言之，MyTDS正式ARCHIEVE了，是时候迈向新的项目了。

不过在这之前，先总结下MyTDS中的不足，在后面的项目多加注意吧。
代码层面：
- 战斗模块耦合太高，gameplay和UI耦合在一起，不应该。
- 很多加载工具类都依赖于场景上挂节点。
- 资源放的太随意，按模块按类型的都有，找东西不方便
- 技能模块还有迭代空间，加入buff设置。
- 单位初始化没有流程化，可以参考远行星号的单位初始化函数，做类似生命周期的设置，以便更好兼容装备属性。
设计层面：
- 家园模块太鸡肋，摆建筑位置还是塔防或者经营类游戏更合适。
- 资源获取很无趣，拆分道具种类凑抽卡资源真的很无聊。
- 关卡生成太简单，表现也不好。应该完善生成规则，做editor编辑器。
- 关卡选择模块难产了，花了太多时间在搭场景上。
...
> 这个思维导图写的比较乱，末端分支可能是遇到的问题（因为暂时没有经历解决，就先放到这里了）
