...
将技能的释放过程拆解为三个部分后，我们可以任意组合它们，丰富技能逻辑的数量。但对于一些特定的需求（比如我希望有一个投掷物画一个五角星，对投掷物的轨迹上的敌人造成伤害）却不能通过数据配置，而且专门为它设计一个`Selector`类好像也不太值当（因为重用的可能不高，还得重新编译），所以我们可以尝试配合热更新，用**外部代码**实现。
